...
let httpRequestDurationMilliseconds;

export function initialize() {
  defaultMetricsInterval = Prometheus.collectDefaultMetrics();

  httpRequestDurationMilliseconds = new Prometheus.Histogram({
  ...
  const tasksInRequestQueuePendingDurationMilliseconds = new Prometheus.Histogram(
    {
      ...
      help:
        'Duration of task pendings in request process queue in ms (waiting time)',
      // labelNames: ['type'],
      buckets: [0.1, 5, 15, 50, 100, 200, 300, 400, 500], // buckets for response time from 0.1ms to 500ms
    }
  );
