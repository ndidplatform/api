...
if (process.env.DEFAULT_API_VERSION != null) {
  if (
    ...
    console.error(
      'ERROR:',
      'Unsupported "DEFAULT_API_VERSION" environment variable value. Only "4.0" and "5.2" are allowed. Process will now exit.'
    );
    process.exit(1);
  }
}
...
if (process.env.CALLBACK_API_VERSION != null) {
  if (
    ...
    console.error(
      'ERROR:',
      'Unsupported "CALLBACK_API_VERSION" environment variable value. Only "4.0" and "5.2" are allowed. Process will now exit.'
    );
    process.exit(1);
  }
}
