import PeerId from 'peer-id';
import PeerInfo from 'peer-info';
import Node from './libp2p-bundle';
import pull from 'pull-stream';
import Async from 'async';
import EventEmitter from 'events';
import * as config from '../config';
import * as utils from '../main/utils';

export const eventEmitter = new EventEmitter();

// read configuration
var peer = null;
if (config.role == "idp") peer = {
    "id": "Qma3GsJmB47xYuyahPZPSadh1avvxfyYQwk8R3UnFrQ6aP",
    "privKey": "",
    "pubKey": ""
}
else if (config.role == "rp") peer = {
    "id": "QmcrQZ6RJdpYuGvZqD5QEHAv6qX4BrQLJLQPQUrTrzdcgm",
    "privKey": "",
    "pubKey": ""
}
// initialize node
var peerNode;
PeerId.createFromJSON(peer, (err, id) => {
    if (err) { return err; }
    console.log('create peerId');
    const peerInfo = new PeerInfo(id);
    peerInfo.multiaddrs.add(`/ip4/0.0.0.0/tcp/${config.peerRegister.port}`)
    peerNode = new Node(peerInfo);
    console.log('create peerNode');

    peerNode.switch.on('peer-mux-established', (pf) => {
        console.log('received dial to me from:', pf.id.toB58String())
    })
    
    peerNode.handle('/data/1.0.0', (protocol, conn) => {
        console.log('set handler');
        pull(
            conn,
            pull.map((v) => v.toString()),
            pull.collect((err, data) => {
                // handle data here
                console.log(data)
            }));
    });

    peerNode.start(err => {
        if (err) { throw err }
        console.log('node start');
    });
})

export const send = async (receivers, message) => {
    Async.each(receivers, async (receiver, cb) => {
        console.log(receiver);
        await sendMessage(receiver, message)
        //cb();
      }, err => {
        console.log(err);
      });
}

async function sendMessage(receiver, message) {
    // initialize listener peer
    PeerId.createFromJSON(receiver, (err, listenerId) => {
        var listenerInfo = new PeerInfo(listenerId);
        listenerInfo.multiaddrs.add(`/ip4/${receiver.ip}/tcp/${receiver.port}`);
        // TODO proper encrypt
        // let encryptedMessage = await utils.encryptAsymetricKey(
        //     receiver.pubKey,
        //     JSON.stringify(message)
        // );
        peerNode.dialProtocol(listenerInfo, '/data/1.0.0', (err,conn) => {
            console.log('dial to ' + listenerId.toB58String());
            pull(
                pull.values([message]),
                conn
            );
        });
    })
}